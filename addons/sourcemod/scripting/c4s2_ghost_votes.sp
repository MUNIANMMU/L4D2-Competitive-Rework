...
public void OnAllPluginsLoaded()
{
	...
	g_hHumanHasThermal	= FindConVar("c4s2_ghost_human_thermal");
}
...
void GhostVoteMenu(int client)
{
	...
	Format(iteam, sizeof(iteam), "%s", g_hHumanHasThermal.BoolValue ? "禁用人类热力机制" : "启用人类热力机制");
	menu.AddItem("7", iteam);
...
void ChangeHumanHasThermal(int client)
{
	char oname[128];
	GetClientOriginalName(client, oname, sizeof(oname));
	if (g_hHumanHasThermal.BoolValue)
	{
		Format(g_sBuffer[0], 128, "c4s2_ghost_human_thermal 0");
		Format(g_sBuffer[1], 128, "已禁用人类热力机制。");
		char title[128];
		Format(title, sizeof(title), "%s: 禁用人类热力机制?", oname);
		s_CallVote(client, title, VoteResult);
	}
	else
	{
		Format(g_sBuffer[0], 128, "c4s2_ghost_human_thermal 1");
		Format(g_sBuffer[1], 128, "已启用人类热力机制。人类开枪会增加热值, 热值达到100后进入过热状态, 无法开枪。热力会自然降低。");
		char title[128];
		Format(title, sizeof(title), "%s: 启用人类热力机制?", oname);
		s_CallVote(client, title, VoteResult);
	}
}
