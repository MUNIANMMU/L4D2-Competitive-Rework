...
public void OnPluginStart()
{
	...
	CreateNative("IsClientInGhost", Native_IsClientInGhost);
	CreateNative("IsClientInSoldier", Native_IsClientInSoldier);
	CreateNative("C4S2Ghost_GetClientTeam", Native_C4S2Ghost_GetClientTeam);
	CreateNative("C4S2Ghost_SetClientTeam", Native_C4S2Ghost_SetClientTeam);
	CreateNative("C4S2Ghost_GetMemberList", Native_GetMemberList);
...
any Native_IsClientInGhost(Handle plugin, int numParams)
{
	return IsClientInGhost_Self(GetNativeCell(1));
}

any Native_IsClientInSoldier(Handle plugin, int numParams)
{
	return IsClientInSoldier_Self(GetNativeCell(1));
}

any Native_C4S2Ghost_GetClientTeam(Handle plugin, int numParams)
{
	return GetClientTeam_Self(GetNativeCell(1));
}

any Native_C4S2Ghost_SetClientTeam(Handle plugin, int numParams)
{
	int	 client	 = GetNativeCell(1);
	bool setglow = GetNativeCell(3);
	if (!(client > 0 && client <= MaxClients) || !IsClientInGame(client)) return;
	char userid[32];
	FormatEx(userid, 32, "%d", GetClientUserId(GetNativeCell(1)));
	SetTrieValue(g_hMembers, userid, GetNativeCell(2));
	if (!setglow) return;
	if (GetNativeCell(2) == 2)
	{
...
any Native_GetMemberList(Handle plugin, int numParams)
{
	ArrayList al		= GetNativeCell(2);
	int		  team		= GetNativeCell(1);
	bool	  aliveonly = GetNativeCell(3);
	int		  membercount;

	for (int i = 0; i < GetTrieSize(g_hMembers); i++)
	{
		char userid[32];
		int	 teaminmap;
		GetTrieSnapshotKey(g_hMembers, i, userid, 32);
		int client = GetClientOfUserId(StringToInt(userid));
		GetTrieValue(g_hMembers, userid, teaminmap);
		if (client != 0 && teaminmap == team)
		{
			if (!aliveonly || (aliveonly && IsPlayerAlive(client)))
			{
				if (al != INVALID_HANDLE)
				{
					al.Push(StringToInt(userid));
				}
				membercount++;
			}
		}
...
Action OnTakeDamage_CallBack(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	//拦截幽灵友伤。
	if ((IsClientInGhost_Self(victim) && IsClientInGhost_Self(attacker)) && victim != attacker)
	{
		return Plugin_Handled;
	}
	...
	if (health - RoundFloat(damage) > 0)
	{
		...
		//对受击的生还施加无敌
		if (IsClientInSoldier_Self(victim) && (damagetype & DMG_MELEE))
		{
...
float OverRide_MeleeDamageCount(int victim, int &attacker, int damagetype, int &weapon)
{
	float damage = 50.0;
	//是否误伤队友,幽灵友伤系数0
	if (GetClientTeam_Self(victim) == GetClientTeam_Self(attacker))
	{
		damage = 0.0;
	}
	//是否爆头或背刺
...
float OverRide_BulletDamageCount(int victim, int &attacker, int damagetype, int &weapon, float damagePosition[3])
{
	...
	//是否误伤队友,生还友伤系数0.25
	if (GetClientTeam_Self(victim) == GetClientTeam_Self(attacker) && victim != attacker)
	{
		damage *= 0.25;
	}
	//是否爆头
...
int MemberStatistics(int team)
{
	...
	{
		if (IsClientInGame(i) && !IsFakeClient(i) && GetClientTeam_Self(i) == team && IsPlayerAlive(i))
		{
			count++;
		}
	}
...
bool IsClientInGhost_Self(int client)
{
	if (!(client > 0 && client <= MaxClients) || !IsClientInGame(client)) return false;
	int	 clientid = GetClientUserId(client);
	char sBuffer[32];
	IntToString(clientid, sBuffer, 32);
	int team;
	return GetTrieValue(g_hMembers, sBuffer, team) ? team == 3 : false;
}

bool IsClientInSoldier_Self(int client)
{
	if (!(client > 0 && client <= MaxClients) || !IsClientInGame(client)) return false;
	int	 clientid = GetClientUserId(client);
	char sBuffer[32];
	IntToString(clientid, sBuffer, 32);
	int team;
	return GetTrieValue(g_hMembers, sBuffer, team) ? team == 2 : false;
}

int GetClientTeam_Self(int client)
{
	...
	IntToString(clientid, sBuffer, 32);
	int team;
	return GetTrieValue(g_hMembers, sBuffer, team) ? team : 1;
}