...
void OnSayToAll(int client, const char[] sArgs)
{
	...
	{
		Format(sInfo, sizeof(sInfo), "*旁观者* {green}%N{default}：%s", client, sArgs);
	}
	else if (IsClientInGhost(client))
	{
		Format(sInfo, sizeof(sInfo), "{blue}%N{default}：%s", client, sArgs);
	}
...
void OnSayToTeam(int client, const char[] sArgs)
{
	...
	{
		Format(sInfo, sizeof(sInfo), "{default}({green}幽灵{default}) {green}%N{default}：%s", client, sArgs);
	}
	else if (IsClientInSoldier(client))
	{
		Format(sInfo, sizeof(sInfo), "{default}({blue}人类{default}) {blue}%N{default}：%s", client, sArgs);
	}
...
{
	if (g_hTransformMode.BoolValue) return;
	for (int i = 1; i <= MaxClients; i++)
	{
