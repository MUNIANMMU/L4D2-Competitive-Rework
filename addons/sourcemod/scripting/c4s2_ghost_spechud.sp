...
void FillSoldiersInfo(Handle hud)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		if (IsPlayerAlive(i))
		{
			...
			if (IsValidEntity(weapon)) GetEntityClassname(weapon, weaponclassname, 64);
		...
		Format(info, sizeof(info), "%s | %N | %s", hp, i, weaponinfo);
...
void FillGhostsInfo(Handle hud)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		Format(info, sizeof(info), "%s | %N | %s", hp, i, moveinfo);
		DrawPanelText(hud, info);
	}
}
